...
contract("TicTacToe", function (accounts) {
  ...
  it("should require a minimum bet to register", async function () {
    await catchRevert(game.registerX({from: alice, value: 0}));
    await catchRevert(game.registerX({from: alice, value: 10}));
    await catchRevert(game.registerO({from: bob, value: 0}));
    await catchRevert(game.registerO({from: bob, value: 10}));
  });

  it("should allow to register players", async function () {
    await game.registerX({from: alice, value: 1e18});
    assert.equal(await game.playerX(), alice);
    
    await game.registerO({from: bob, value: 1e18});
  ...
  it("should prevent players from registering more than once", async function () {
    await game.registerX({from: alice, value: 1e18});
    assert.equal(await game.playerX(), alice);
    await catchRevert(game.registerX({from: alice, value: 1e18}));
    await catchRevert(game.registerX({from: bob, value: 1e18}));
    
    await game.registerO({from: bob, value: 1e18});
    assert.equal(await game.playerO(), bob);
    await catchRevert(game.registerO({from: alice, value: 1e18}));
    await catchRevert(game.registerO({from: bob, value: 1e18}));
  });
